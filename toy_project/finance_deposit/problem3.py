...
def get_deposit_products():
  ...
  new_list = []
  for i in range(40):
    new_dict={'금융상품코드': deposit_data['result']['baseList'][i]['fin_prdt_cd'],
              '저축 금리':deposit_data['result']['optionList'][i]['intr_rate'],
              '저축 기간':deposit_data['result']['optionList'][i]['save_trm'],
              '저축금리유형':deposit_data['result']['optionList'][i]['intr_rate_type'],
              '저축금리유형명':deposit_data['result']['optionList'][i]['intr_rate_type_nm'],
              '최고 우대금리':deposit_data['result']['optionList'][i]['intr_rate2']
              }
